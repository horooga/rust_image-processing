...
pub fn get_n_index(v: String) -> usize {
    let mut c: i32 = 0;
    for i in 0..v.len() {
...
pub fn get_v_indexes(line: Vec<String>) -> Vec<usize> {
...
pub fn rgb_dist(one: [i32; 3], two: [i32; 3]) -> u16 {
...
pub fn closest_color(colors_from: Vec<[i32; 3]>, color_to: [i32; 3]) -> Rgb<u8> {
...
pub fn closest_index(max_index: usize, val_to: f32) -> usize {
    let mut mindist: f32 = 65535.0;
    let mut dist: f32;
    let mut index: usize = 0;
    for ci in 0..max_index {
        dist = (val_to - ci as f32).abs();
        if dist < mindist {
            mindist = dist;
            index = ci;
        }
...
pub fn rec_to_quad(img: &mut ImageBuffer<Rgb<u8>, Vec<u8>>) -> ImageBuffer<Rgb<u8>, Vec<u8>> {
    let (x, y) = img.dimensions();
    let size: u32 = cmp::min(x, y);
    let offset: u32 = (x - size) / 2;
    let mut nimg: ImageBuffer<Rgb<u8>, Vec<u8>> = RgbImage::new(size, size);
    for i in 0..size {
        for j in 0..size {
            nimg.put_pixel(j, i, *img.get_pixel(j + offset, i));
        }
    }
    nimg
}
